...
update_articles_in_DB_presentation_dag = DAG(
    dag_id='transform_periodic_update_presentation',
...
def update_publications():
    ...
    publications_old_df = pd.read_sql(
        'SELECT publication_id, doi, number_of_citations FROM publications WHERE doi IS NOT NULL LIMIT 200', conn)
    publications_old_df['number_of_citations_new'] = publications_old_df.apply(lambda x: do_opencitation_call(x.doi),
                                                                               axis=1)
    publications_new_df = publications_old_df[
        publications_old_df.number_of_citations_new.gt(publications_old_df.number_of_citations)]
