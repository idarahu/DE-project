...
def find_venue(venue_data_raw, venues_lookup):
    ...
    try:
        ...
        found_venue1 = venues_lookup[['abbrev.dots', 'full']].to_numpy()[
            venues_lookup['abbrev_check'].to_numpy() == venue_data_check].tolist()
        found_venue2 = venues_lookup[['abbrev.dots', 'full']].to_numpy()[
            venues_lookup['full_check'].to_numpy() == venue_data_check].tolist()
...
def get_venues_and_publications_data():
    ...
    metadata_df[['venue_abbr', 'venue_name']] = metadata_df.apply(
        lambda x: find_venue(x.venue_data_raw, venues_lookup) if x.venue_data_raw is not None else [None, None], axis=1,
        result_type='expand')
    crossref_columns = ['type', 'language', 'issue_cross', 'volume_cross', 'number_of_references', 'venue_name_cross',
                        'venue_abbr_cross', 'print_issn_pub', 'electronic_issn_pub']
    metadata_df[crossref_columns] = metadata_df.apply(lambda x: do_crossref_call(x.doi, DOIs_for_enrichment), axis=1,
                                                      result_type='expand')
    opencitation_columns = ['venue_name_open', 'volume_open', 'issue_open', 'page_numbers', 'number_of_citations',
                            'citing_publication_DOI']
    metadata_df[opencitation_columns] = metadata_df.apply(lambda x: do_opencitation_call(x.doi, DOIs_for_enrichment),
                                                          axis=1, result_type='expand')
    metadata_df['issue'] = metadata_df.apply(lambda x: x.issue_cross if x.issue_cross != -1 else x.issue_open, axis=1)
    metadata_df['volume'] = metadata_df.apply(lambda x: x.volume_cross if x.volume_cross != -1 else x.volume_open,
                                              axis=1)
    metadata_df[['venue_abbr', 'venue_name']] = metadata_df.apply(
        lambda x: check_API_venues(x.venue_name_cross, x.venue_abbr_cross, x.venue_name_open,
                                   venues_lookup) if x.venue_abbr is None else [x.venue_abbr, x.venue_name], axis=1,
        result_type='expand')
    merged_df = pd.merge(left=metadata_df, left_on=['venue_abbr', 'venue_name'], right=venues_df,
                         right_on=['abbreviation', 'full_name'], how='outer')
    ...
    publications_df = merged_df[
        ['publication_ID', 'venue_ID', 'doi', 'title', 'date', 'submitter', 'type', 'language', 'page_numbers',
         'volume', 'issue',
    ...
    publications_df.applymap(lambda x: None if x == '' else x)
    columns = ['publication_ID', 'venue_ID', 'volume', 'issue', 'number_of_references', 'number_of_citations',
               'no_versions_arxiv']
...
def get_arxiv_category_data():
    ...
    metadata_df = pd.read_table(f'{METADATA_FOLDER}/metadata_df.tsv')
    metadata_df.drop(
        ['submitter', 'authors', 'title', 'journal_ref', 'doi', 'no_versions_arxiv', 'date_of_first_version', 'date'],
        axis=1, inplace=True)
    metadata_df['categories'] = metadata_df['categories'].apply(literal_eval)
    metadata_df = metadata_df.explode('categories')
    merged_df = pd.merge(left=metadata_df, left_on='categories', right=arxiv_categories, right_on='arxiv_category',
                         how='left')
    publication2arxiv_df = merged_df[['publication_ID', 'arxiv_category_ID']]
    publication2arxiv_df.applymap(lambda x: None if x == ' ' else x)
    publication2arxiv_df.applymap(lambda x: None if x == '' else x)
    publication2arxiv_df[['publication_ID', 'arxiv_category_ID']] = publication2arxiv_df[
        ['publication_ID', 'arxiv_category_ID']].applymap(np.int64)
...
def find_institution_information(institution_name_raw, universities_lookup, cities_lookup):
    ...
    try:
        ...
        if institution_place == None:
            memory2 = None
            for split in splitted_institution_name:
                if cities_lookup['city'].str.lower().str.contains(split.lower()).any() or cities_lookup[
                    'city_ascii'].str.lower().str.contains(split.lower()).any() or cities_lookup[
                    'country'].str.lower().str.contains(split.lower()).any():
                    memory2 = split.lower()

            if memory2 != None:
                for i in range(len(cities_lookup)):
                    if memory2 in cities_lookup.iloc[i]['city'].lower() or memory2 in cities_lookup.iloc[i][
                        'city_ascii'].lower() or memory2 in cities_lookup.iloc[i]['country'].lower():
...
def get_authors_and_affiliations_data():
    ...
    metadata_df = pd.read_table(f'{METADATA_FOLDER}/metadata_df.tsv')
    metadata_df.drop(['submitter', 'title', 'journal_ref', 'doi', 'no_versions_arxiv', 'date_of_first_version', 'date',
    ...
    metadata_df[['first_name', 'first_name_abbr']] = metadata_df.apply(
        lambda x: parse_first_name(x.first_name_raw) if x.first_name_raw is not None else [None, None], axis=1,
        result_type='expand')
    metadata_df[['extra', 'institution_name_raw']] = metadata_df.apply(
        lambda x: extra_or_affiliation(x.raw1, x.raw2) if x.raw1 != '' else [None, None], axis=1, result_type='expand')
    metadata_df[['institution_name', 'institution_place']] = metadata_df.apply(
        lambda x: find_institution_information(x.institution_name_raw, universities_lookup,
                                               cities_lookup) if x.institution_name_raw is not None else [None, None],
        axis=1, result_type='expand')
    ...
    authors_df = metadata_df[
        ['publication_ID', 'last_name', 'first_name', 'first_name_abbr', 'extra', 'position', 'h_index_real']]
    ...
    affiliations_df = metadata_df[
        ['publication_ID', 'institution_name', 'institution_place', 'last_name', 'first_name_abbr']]
    affiliations_df = affiliations_df.rename(
        columns={'last_name': 'author_last_name', 'first_name_abbr': 'author_first_name_abbr'})
    affiliations_df = affiliations_df[
        (affiliations_df.institution_name.notnull()) | (affiliations_df.institution_place.notnull())]
...
def copy_data_from_DB(output_folder, SQL_statement, data_type):
    ...
    if data_type == 'publication2arxiv':
        ...
        connected_domains_data = pd.merge(left=arxiv_categories, how='outer', left_on='arxiv_category',
                                          right=domains_lookup, right_on='arxiv_category')
        final_df = pd.merge(left=connected_domains_data, how='right', left_on='arxiv_category_ID', right=df,
                            right_on='arxiv_category_id')
        final_df_print = final_df[
            ['publication_id', 'domain_id', 'major_field', 'sub_category', 'exact_category', 'arxiv_category']]
        final_df_print[['publication_id', 'domain_id']] = final_df_print[['publication_id', 'domain_id']].applymap(
            np.int64)
