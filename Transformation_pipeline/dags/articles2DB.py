        ...
        for line in f:
            ...
            if parsed_line['doi'] != None or (parsed_line['authors_parsed'] != None and parsed_line['title'] != None):
                metadata_df.loc[len(metadata_df.index)] = [parsed_line['submitter'],
                                                           ...
                                                           parsed_line['doi'],
                                                           datetime.strptime(parsed_line['versions'][-1]['created'],
                                                                             '%a, %d %b %Y %H:%M:%S %Z').strftime(
                                                               '%Y-%m-%d'),
                                                           parsed_line['categories'].split(' '),
                                                           len(parsed_line['versions']),
                                                           datetime.strptime(parsed_line['versions'][0]['created'],
                                                                             '%a, %d %b %Y %H:%M:%S %Z').strftime(
                                                               '%Y-%m-%d')]
            counter += 1
    return metadata_df
...
def find_venue(venue_data_raw):
    ...
    venue_abbr = None
    venue_name = None
    found_venue = venues_lookup.loc[
        (venues_lookup['abbrev_check'] == venue_data_check) | (venues_lookup['full_check'] == venue_data_check)]
...
def transform_and_enrich_the_data():
    ...
    for i in range(len(metadata_df)):
        ...
        if metadata_df.iloc[i]['journal_ref'] != None:
            try:
                venue_data_raw = re.split(r'(^[^\d]+)', metadata_df.iloc[i]['journal_ref'])[1:][0].replace(',',
                                                                                                           '').rstrip()
            except:
                venue_data_raw = None
        ...
        for author in authors:
            ...
            affiliations_temp_df.loc[len(affiliations_temp_df.index)] = [int(publication_ID), institution_name,
                                                                         institution_place,
        ...
        if article_DOI in DOIs_for_enrichment:
            ...
            for author in authors_openc:
                ...
                if len(name_splitted) == 2:
                    ...
                    author_control_index = authors_temp_df.loc[
                        (authors_temp_df['last_name'].str.lower() == last_name_control.lower()) & (authors_temp_df[
                                                                                                       'first_name_abbr'].str.lower() == first_name_abbr_control.lower())].index
                    if len(author_control_index) > 0:
                        ...
                        if first_name != None:
                            authors_temp_df.loc[len(authors_temp_df.index)] = [int(publication_ID), last_name_control,
                                                                               first_name,
                                                                               first_name_abbr_control, None,
                                                                               author_position,
                ...
                try:
                    affiliation_control_index = affiliations_temp_df.loc[
                        (affiliations_temp_df['institution_name'].str.lower() == institution_name.lower())].index
                except:
        ...
        if venue_abbr != None:
            ...
            if len(venue_control_index) > 0:
                ...
                venues_df.loc[len(venues_df.index)] = [int(venue_ID), venue_name, venue_abbr, print_issn,
                                                       electronic_issn]

        publications_df.loc[len(publications_df.index)] = [int(publication_ID), int(venue_ID), article_DOI,
                                                           article_title,
                                                           metadata_df.iloc[i]['date'],
                                                           metadata_df.iloc[i]['submitter'],
                                                           type, language, page_numbers, int(volume), int(issue),
                                                           int(no_references),
...
def copy_data_from_DB(output_folder, SQL_statement, data_type):
    ...
    if data_type == 'publication2arxiv':
        ...
        final_df = pd.merge(left=connected_domains_data, how='outer', left_on='arxiv_category_ID', right=df,
